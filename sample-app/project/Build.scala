...
object IaaSBuild extends Build {
  ...
  lazy val iaas_kernel = Project(
    ...
    libraryDependencies ++= Dependency.base)) dependsOn (iaas_kernel,iaas_datastores,iaas_scripting)
    ...
    settings = defaultSettings ++ Seq(
    libraryDependencies ++= Dependency.base ++ Seq(Dependency.argot,Dependency.spray_base,Dependency.spray_can,Dependency.spray_client,Dependency.lift_json)))
   ...
   lazy val iaas_scripting = Project(
    id = "iaas-scripting",
    base = file("main/scripting"),
    settings = defaultSettings ++ Seq(
    libraryDependencies ++= Dependency.base)) 
  
    lazy val iaas_devkit = Project(
    id = "iaas-devkit",
    base = file("main/devkit"),
    settings = defaultSettings ++ Seq(
    libraryDependencies ++= Dependency.base ++ Seq(Dependency.jline))) dependsOn (iaas_kernel,iaas_engines,iaas_modules,iaas_scripting) 
...
//  val spray_util = "cc.spray" % "spray-util" % V.Spray
  ...
  val lift_json = "net.liftweb" %% "lift-json" % V.LiftJson
